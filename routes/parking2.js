...
router.get('/', function (req, res, next) {
  ...
  var locx = req.session.parkinglocx;
  var locy = req.session.parkinglocy;
  ...
  dbconfig.query(sql, [req.query.cid, cur_time], function (err, results) {
    if (err) {
      ...
      if (results.length == 0) {
        ...
        res.render('parking2', { locx:locx, locy:locy, time: cur_time.getTime(), carstatus: "0", carid: req.query.cid })
      } else {
        //console.log(results)
        console.log("파킹 데이터의 개수 : " + results.length);
        res.render('parking2', { locx:locx, locy:locy, time: cur_time.getTime(), state: results[results.length - 1]["carstatus"], carid: req.query.cid })
      }
    }
  });
